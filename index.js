...
app.post("/schedules", async(req, res) => {
    try{
        ...
        const newSchedules = await pool.query("INSERT INTO schedules(description) VALUES($1) RETURNING * ", 
        [description])
...
app.put("/schedules/:id", async (req, res)=> {
    try {
        const {id} = req.params;
        const {description} = req.body;
        const upDate = await pool.query("UPDATE schedules SET description = $1 WHERE schedule_id = $2 ", 
        [description, id]
        );
...
app.delete("/schedules/:id", async(req, res) => {
    try {
       const { id } = req.params;
       const deleteSchedule  = await pool.query("DELETE FROM schedules WHERE schedule_id = $1", [id])
